<?PHP
 include ("../include/login.inc.php");
 include ($header_filename);
?>
<h1>SDL Documentation</h1>

<p class="intro">
SDL documentation is available online from the sidebar on the left,
or downloadable as archives from the links on this page.
</p>

<h2>Introduction:</h2>
<blockquote>
     <a href="archives/SDL-1.0-chinese-intro.tar.gz"
                      >SDL-1.0-chinese-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-chinese-intro.zip"
                      >SDL-1.0-chinese-intro.zip</a><br>
     <a href="archives/SDL-1.0-czech-intro.tar.gz"
                      >SDL-1.0-czech-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-czech-intro.zip"
                      >SDL-1.0-czech-intro.zip</a><br>
     <a href="archives/SDL-1.0-english-intro.tar.gz"
                      >SDL-1.0-english-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-english-intro.zip"
                      >SDL-1.0-english-intro.zip</a><br>
     <a href="archives/SDL-1.0-french-intro.tar.gz"
                      >SDL-1.0-french-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-french-intro.zip"
                      >SDL-1.0-french-intro.zip</a><br>
     <a href="archives/SDL-1.0-german-intro.tar.gz"
                      >SDL-1.0-german-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-german-intro.zip"
                      >SDL-1.0-german-intro.zip</a><br>
     <a href="archives/SDL-1.0-italian-intro.tar.gz"
                      >SDL-1.0-italian-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-italian-intro.zip"
                      >SDL-1.0-italian-intro.zip</a><br>
     <a href="archives/SDL-1.0-japanese-intro.tar.gz"
                      >SDL-1.0-japanese-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-japanese-intro.zip"
                      >SDL-1.0-japanese-intro.zip</a><br>
     <a href="archives/SDL-1.0-korean-intro.tar.gz"
                      >SDL-1.0-korean-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-korean-intro.zip"
                      >SDL-1.0-korean-intro.zip</a><br>
     <a href="archives/SDL-1.0-portuguese-intro.tar.gz"
                      >SDL-1.0-portuguese-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-portuguese-intro.zip"
                      >SDL-1.0-portuguese-intro.zip</a><br>
     <a href="archives/SDL-1.0-russian-intro.tar.gz"
                      >SDL-1.0-russian-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-russian-intro.zip"
                      >SDL-1.0-russian-intro.zip</a><br>
     <a href="archives/SDL-1.0-spanish-intro.tar.gz"
                      >SDL-1.0-spanish-intro.tar.gz</a><br>
     <a href="archives/SDL-1.0-spanish-intro.zip"
                      >SDL-1.0-spanish-intro.zip</a><br>
</blockquote>

<h2>SDL Documentation Project:</h2>
<blockquote>
<p>
HTML documentation:<br>
     <a href="archives/sdldoc-html.tar.gz"
                      >sdldoc-html.tar.gz</a><br>
     <a href="archives/sdldoc-html.zip"
                      >sdldoc-html.zip</a>
</p>
<p>
UNIX "man" pages:<br>
     <a href="archives/sdldoc-man3.tar.gz"
                      >sdldoc-man3.tar.gz</a>
</p>
<p>
Windows Help format: (thanks to Lea Anthony)<br>
     <a href="archives/SDLRef.chm"
                      >SDLRef.chm</a>
</p>
<p>
Printable Postscript:<br>
     <a href="archives/sdldoc-ps.tar.gz"
                      >sdldoc-ps.tar.gz</a><br>
     <a href="archives/sdldoc-ps.zip"
                      >sdldoc-ps.zip</a>
</p>
</blockquote>

<h2>Frequently Asked Questions:</h2>
<blockquote>
     <a href="archives/SDL-1.2-FAQ.tar.gz"
                      >SDL-1.2-FAQ.tar.gz</a><br>
     <a href="archives/SDL-1.2-FAQ.zip"
                      >SDL-1.2-FAQ.zip</a>
</blockquote>

<?PHP
 include ("footer.inc.php");
?>
